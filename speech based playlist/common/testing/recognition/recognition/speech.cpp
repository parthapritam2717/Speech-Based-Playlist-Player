...
long double energy_frame[20000];// to store the energy of each frame
...
char source[200] = { '\0' };	// to store the source universe file address
...
long double dist[200000] = { 0 };   // to store the min distance for the code vector
...
char test[200];
int utter[20] = { 0 };
int words;// will have to minus the values
int identified;
...
void generate_codebook(){
	...
	noOfLine = read_universe(source);  // now we have our universe in array and total number of vectors 
...
void load_config(){
	...
	while (fgets(bufconf, 100, fp)){
		...
		if (strcmp(temp, "source") == 0){
			strncpy(source, bufconf + index + 1, len - index - 2);
		}
...
void purge_log_files(){
	...
	fclose(fp);
	// to overwrite previous individual files
	/*for (int i = 0; i <= 9; i++){
	for (int j = 1; j <= 30; j++){
	// reset all global variables
	update_suffix_file_name(i, j);
	char address[200] = "E:\\project\\output\\individual_vector\\154101026_";
	...
	strcat(input_voice, ".txt");
	//printf("%s\n", input_voice);
	FILE *fps = openfile(input_voice, "w");
	...
	for (int i = 0; i <= 9; i++){
		for (y = 0; y <= 2; y++){
			...
			for (int j = 1; j <= 20; j++){
				...
				char *token;
				update_suffix_file_name(i, j);
				char address[200] = "E:\\common\\output\\observation\\154101026_";
		...
		long double sum = 0;
		printf("Printing the A row sum for digit %d:\n", i);
		for (int i = 0; i < N; i++){
			sum = 0;
			for (int j = 0; j < N; j++){
		...
		printf("Printing the B row sum for digit %d:\n", i);
		for (int i = 0; i < N; i++){
			sum = 0;
			for (int j = 0; j < Mh; j++){
		...
		fp = openfile(input_voice, "w");
		int sum1 = 0;
		printf("Printing the P row sum for digit %d:\n", i);
		sum = 0;
		for (int e = 0; e < N; e++){
	...
	if (dp != NULL)
	{
		while (ep = readdir(dp)){
			...
			for (int z = 0; z <= words - 1; z++){
				...
				problem1_calc();
				/*update the log file for testing which is created in the common folder */
			char addd[150]= "E:\\common\\testing_log.txt";
				fp = openfile(addd, "a");
				fprintf(fp," percentage match with word %d =%E\n",z,P);
				fclose(fp);
				if (P>Pmax){
					Pmax = P;
					max_number = z;
...
void preprocess(){
	/*read audio from test*/
	DIR *dp;
	struct dirent *ep;
	char temp[200];
	dp = opendir("E:/common/output/test");
	if (dp != NULL)
	{
		while (ep = readdir(dp)){
			if (ep->d_name[0] == '.' || ep->d_name[0] == '.'){
			...
			memset(temp, '\0', sizeof(char));
			memset(input_voice, '\0', sizeof(char));
			strcpy(temp, "E:/common/output/test/");
			strcat(temp, ep->d_name);
			strcpy(input_voice, temp);
			//puts(temp);
			read_source_voice(temp);
			...
			for (int z = 0; z <= noOfLine_speech - 320; z += shift){
				...
				cepstral_ci();
				char address[200];
				strcpy(address, "E:/common/output/test_ci/");
				//"E:\\project\\output\\individual_vector\\";			
				strcat(address, ep->d_name);
				FILE *fps = openfile(address, "a");
				//fprintf(fps, "Ci=,");
				for (int i = 1; i <= p; i++){
					//printf("\nC%d=%LLf\n", i, C[i]);
	...
	/*now create observation vector for testing file*/
	char ind[200];
	strcpy(ind, "E:/common/output/test_ci/");
	dp = opendir(ind);
	if (dp != NULL)
	{
		while (ep = readdir(dp)){
			...
			fclose(fps);
			// read the codebook
			FILE *fcodebook = openfile("E:\\common\\output\\codebook.txt", "r");
			int lines = 0;
			int j = 0, i = 0;
			char *token;
			char buf[600] = { '\0' };
			//FILE *fp = openfile(s, "r");
			while (fgets(buf, 600, fcodebook)){//&& lines<3
				lines += 1;
				j = 0;
				token = strtok(buf, delim);
				while (token != NULL) {
					double number = atof(token);
					codebook[lines - 1][j] = number;
					//printf("%lf\n", source[noOfLine-1][j]);
					j++;
					token = strtok(NULL, delim);
				}
			...
			//print_ob(ep->d_name);
			char address[200] = "E:/common/output/testing/";
			strcat(address, ep->d_name);
			FILE *f = openfile(address, "w");
			//fprintf(fps, "Ci=,");
			for (int i = 0; i <temp_obs_len; i++){
...
void deletefiles(){
	/*delete previous files in testing ci,testing observation folder*/
	char temp[200];
	char ind[200];
	strcpy(ind, "E:\\common\\output\\test_ci\\");
	DIR *dp;
	struct dirent *ep;
	FILE *fp;
	dp = opendir(ind);
	...
	memset(temp, '\0', sizeof(char));
	dp = opendir("E:\\common\\output\\testing\\");
	if (dp != NULL){
		while (ep = readdir(dp)){
			if (ep->d_name[0] == '.' || ep->d_name[0] == '.'){
				continue;
			...
			memset(temp, '\0', sizeof(char));
			memset(input_voice, '\0', sizeof(char));
			strcpy(temp, "E:\\common\\output\\testing\\");
			strcat(temp, ep->d_name);
			strcpy(input_voice, temp);
			remove(input_voice);
		}
...
int main(){
	...
	deletefiles();
	char add[150] = "E:\\common\\testing_log.txt";
